...
# Single unit will be viewed as a special case of multi units.
class MultiUnitMultiHeadPdType(PdType):
    def __init__(self, size_shape):
        self.unit_count = size_shape[0]
        self.multi_head_shape = size_shape[1:]
        self.head_count = len(self.multi_head_shape)

    def pdclass(self):
        return MultiUnitMultiHeadPd
    ...
    def param_shape(self):
        raise NotImplementedError
        return [2*self.multi_head_shape]

    def sample_shape(self):
        return [self.unit_count, self.head_count]
...
class MultiUnitMultiHeadPd(Pd):
    ...
    def neglogp(self, label_actions):
        # Todo: need to mask the logp output
        total_neglogp = 0
        unit_count = len(self.logits)
        for unit_idx in range(unit_count):
            logits_arr = self.logits[unit_idx]
            head_count = len(logits_arr)
            for head_idx in range(head_count):
                x = label_actions[:, unit_idx, head_idx]
                logits = logits_arr[head_idx]
                # return tf.nn.sparse_softmax_cross_entropy_with_logits(logits=self.logits, labels=x)
                # Note: we can't use sparse_softmax_cross_entropy_with_logits because
                #       the implementation does not allow second-order derivatives...
                if x.dtype in {tf.uint8, tf.int32, tf.int64}:
    ...
    def kl(self, other):
        ...
        for unit_idx in range(unit_count):
            ...
            for head_idx in range(len(multi_heads)):
                other_logits = other.logits[unit_idx][head_idx]
                ...
                z1 = tf.reduce_sum(ea1, axis=-1, keepdims=True)
                p0 = ea0 / z0
                _kl = tf.reduce_sum(p0 * (a0 - tf.log(z0) - a1 + tf.log(z1)), axis=-1)
                total_kl += _kl
        return total_kl
    ...
    def entropy(self):
        ...
        for unit_idx in range(unit_count):
            ...
            for head_idx in range(len(multi_heads)):
                ...
                z0 = tf.reduce_sum(ea0, axis=-1, keepdims=True)
                p0 = ea0 / z0
                _entropy = tf.reduce_sum(p0 * (tf.log(z0) - a0), axis=-1)
    ...
    def sample(self):        
        ...
        unit_actions = tf.stack(unit_actions, axis = 1)
        return unit_actions
...
def make_pdtype(ac_space):
    ...
    if isinstance(ac_space, spaces.Box):
        if len(ac_space.shape) == 1:
            return DiagGaussianPdType(ac_space.shape[0])
        else:
            # Add multi head support
            return MultiUnitMultiHeadPdType(ac_space.shape)
